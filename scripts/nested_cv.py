...
Runs an inner GridSearchCV (3-fold) for hyperparameter selection and an
outer StratifiedKFold (5-fold) for unbiased performance estimation.
Called per model via Snakemake wildcard {model}.
"""
...
# create inner and outer cross validation with 3 and 5 splits respectively
...
inner = GridSearchCV(
    ...
    cv=inner_cv,
    scoring="f1_macro",
    n_jobs=1  # parallelism is handled by the outer cross_val_score
    )
...
# write outer CV scores to log and save summary CSV for comparison
